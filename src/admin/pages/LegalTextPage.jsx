...
import { parsePrivacy } from '../../data/privacy';
...
/**
 * Generic legal-text editor used by the Legals section (Terms of Use, GDPR,
 * Cookie Policy). Same behaviour as PrivacyPage, parameterised by document:
 * the DB row key, the built-in defaults, the public-page labels for the
 * preview and the public route.
 *
 * Text identical to the built-in default is stored as NULL, so future code
 * updates to defaults keep flowing through until the admin actually edits.
 */
export default function LegalTextPage({ textKey, title, publicHref, labels, defaults }) {
  ...
  // Reset editor state when switching between documents sharing this route element.
  useEffect(() => {
    setText(null); setSaved(null); setError(''); setLang('ru');
    let cancelled = false;
    getSiteText(textKey)
      .then(row => {
        if (cancelled) return;
        const next = {};
        LANGS.forEach(({ code }) => { next[code] = (row && row[code]) || defaults[code]; });
        setText(next);
  ...
  const dirty = useMemo(() => {
    ...
    return LANGS.some(({ code }) => {
      ...
      return cur !== was && !(was === '' && cur === defaults[code].trim());
    });
  }, [text, saved, defaults]);

  const preview = useMemo(() => text ? parsePrivacy(text[lang]) : null, [text, lang]);
  const isDefault = text ? text[lang].trim() === defaults[lang].trim() : true;
  ...
  async function save() {
    ...
    try {
      ...
      LANGS.forEach(({ code }) => {
        ...
        payload[code] = v === defaults[code].trim() ? null : v;
      });
      const row = await saveSiteText(textKey, payload);
  ...
  function resetLang() {
    setText(t => ({ ...t, [lang]: defaults[lang] }));
    setResetOpen(false);
  }
  ...
  return (
    <>
      <div className="adm-head">
        <div>
          <div className="adm-eyebrow">Legals</div>
          <h1>{title}</h1>
          <p className="adm-lede" style={{ marginBottom: 0 }}>
            Страница <code>{publicHref}</code>. Три языка, каждый редактируется отдельно. Пустой язык на сайте показывает встроенный текст.
          </p>
        </div>
        <div className="adm-head-actions">
          <a className="adm-btn adm-btn--sm adm-btn--ghost" href={publicHref} target="_blank" rel="noopener noreferrer">
        ...
        <>
          ...
          <div className="adm-tabs">
            {LANGS.map(l => (
              ...
              >
                {l.label}
                {text[l.code].trim() !== defaults[l.code].trim() && <i className="adm-tab-dot" title="Изменён" />}
              </button>
            ))}
          </div>
          ...
          <div className="adm-privacy-grid">
            ...
            <div className="adm-panel adm-privacy-preview">
              ...
              <div className="adm-privacy-page">
                <p className="adm-privacy-eyebrow">{labels[lang].eyebrow}</p>
